...
namespace BetterMatchMaking.Sample
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var dataset = test.Load("..\\..\\..\\BetterMatchMaking.UI\\Bin\\Debug\\petit-le-mans-2019-fieldsize45.csv");
            ...
            calculator.Calculator.ParameterPValue = 37;
            calculator.Calculator.ParameterMaxSofDiff = 18;
            calculator.Calculator.ParameterMaxSofFunctA = 12;
            calculator.Calculator.ParameterMaxSofFunctX = 1000;
            calculator.Calculator.ParameterMaxSofFunctB = -20;
            calculator.Calculator.ParameterTopSplitException = 1;

            // 3 : Launch
            int fieldSize = 45;
            calculator.Calculator.Compute(dataset, fieldSize);

            // 4 : Display Results in console
            foreach (var split in calculator.Calculator.Splits)
            {
        ...
        public List<BetterMatchMaking.Library.Data.Line> Load(string csvFile)
        {
            var parser = new BetterMatchMaking.Library.Data.CsvParser();

            
            // Here we will read data from CSV a file
            parser.Read(csvFile);
            return parser.DistinctCars; // and return the result
            


            // but you can build the entry list from Code
